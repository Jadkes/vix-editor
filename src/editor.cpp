...
static int fuzzy_score(std::string_view q, std::string_view s) {
    size_t qi = 0;
    int score = 0;
...
/* Filename extension -> lowercased string (no dot). Built in a new buffer so
 * callers can keep the result past the lifetime of the input. */
static std::string file_ext(std::string_view fname) {
    size_t dot = fname.find_last_of('.');
    if (dot == std::string_view::npos || dot == fname.length() - 1) return "";
...
/* Run {args} with working directory {wd}, no shell, and wait for exit.
 * Returns the child's exit code, or -1 on failure to spawn.
 * Takes a span so any contiguous string container (vector, initializer list)
 * can be passed; execvp needs mutable char* argv members, so the span is
 * non-const and data() yields char* without a cast. */
static int run_process(std::span<std::string> args, const std::string& wd) {
...
static size_t ifind(std::string_view hay, std::string_view needle, size_t pos) {
...
void Editor::load_session() {
    ...
    while (i < end) {
        ...
        if (key == "current_dir" && i < end && text[i] == '"') { i++; saved_dir = read_str(); have_dir = true; }
            ...
            auto [ptr, ec] = std::from_chars(val.data(), val.data() + val.size(), saved_tab);
            (void)ptr; (void)ec;
        }
        else if (key == "files") {
            if (i < end && text[i] == '[') i++;
      ...
      sidebar_sel(0), sidebar_scroll(0), search_idx(-1)
{
    ...
    update_sidebar();
    set_status(std::format("{} {} - ^H for help", VIX_NAME, VIX_VERSION));
}
...
void Editor::detect_language(Tab& tab) {
    ...
    if (ext_lower == "cpp" || ext_lower == "hpp" || ext_lower == "cc" || ext_lower == "hh" || ext_lower == "cxx" || ext_lower == "hxx")
        tab.rules = {1, "C++",
                     {"return","if","else","for","while","class","public","private","new","delete","const","using","namespace","std","cout","endl","this","virtual","friend","operator"},
                     {"int","void","char","bool","float","double","long","short","signed","unsigned","size_t","auto","string"},
                     {"include","define","ifdef","ifndef","endif","undef","pragma","static"}};
    else if (ext_lower == "c" || ext_lower == "h")
        tab.rules = {6, "C",
                     {"return","if","else","for","while","switch","case","break","default","do","goto","continue","typedef","struct","union","enum","extern","register","volatile","const","sizeof","NULL","FILE","main","printf","malloc","free"},
                     {"int","char","float","double","long","short","signed","unsigned","void","size_t"},
                     {"include","define","ifdef","ifndef","endif","undef","pragma","static"}};
...
void Editor::load_file(Tab& tab, const std::string& fname) {
    ...
    if (!fs::exists(fname, ec)) {
        set_status(std::format("Error: file not found: {}", fname));
        return;
    }
...
bool Editor::save_file(Tab& tab) {
    ...
    set_status(std::format("Error: save failed: {}", buf.GetFilename()));
    return false;
}
...
void Editor::update_sidebar() {
    ...
    try {
        ...
        set_status(std::format("Error reading directory: {}", e.what()));
        sidebar_sel = 0;
        sidebar_scroll = 0;
        return;
    }
...
void Editor::fuzzy_finder() {
    ...
    // Best score first, then name; pair's default sort is (score, name) so
    // negating the scores makes a single std::ranges::sort do both.
    for (auto& [score, name] : scored) score = -score;
    std::ranges::sort(scored);
    std::vector<std::string> matches;
    matches.reserve(scored.size());
    for (auto& [score, name] : scored) matches.push_back(name);
...
void Editor::plain_search(std::string_view line, int line_idx, std::string_view q) {
    size_t pos = 0;
    while (true) {
...
void Editor::replace_current(Tab& tab) {
    ...
    set_status(std::format("Replaced match {}", search_idx + 1));
}
    ...
    while (cy >= 0 && cy < tab.buffer.GetLineCount()) {
        while (cx >= 0 && cx < (int)tab.buffer[cy].length()) {
            if (tab.buffer[cy][cx] == c) depth++;
                ...
                if (depth == 0) { match_pos = std::make_pair(cx, cy); return; }
                depth--;
            }
            cx += dir;
        }
...
void Editor::compile_run(Tab& tab) {
    ...
    std::error_code ec;
    if (!fs::exists(cf, ec)) {
        set_status(std::format("Error: file not found: {}", cf));
        return;
    }
    ...
    if (ext == "cpp" || ext == "cc" || ext == "cxx") {
        ...
        set_status(std::format("No runner for: {}", cf));
        return;
    } else {
        set_status(std::format("Unknown file type: {}", cf));
        return;
    }
...
void Editor::draw_line(int row, int buf_idx, int max_x, int sw, Tab& tab, int start_col, int end_col) {
    ...
    while (i < end_col && cur_x < max_x) {
        ...
        bool is_match = (match_pos.has_value() && match_pos->second == buf_idx && match_pos->first == i);
        char c = line[i];
        ...
        if (c == '"' || c == '\'') {
            ...
            bool is_type = false, is_directive = false, is_kw = false;
            for (auto& k : tab.rules.types) if (k == w) { is_type = true; break; }
            if (!is_type) for (auto& k : tab.rules.directives) if (k == w) { is_directive = true; break; }
            if (!is_type && !is_directive)
                for (auto& k : tab.rules.keywords) if (k == w) { is_kw = true; break; }
            if (is_type) attron(COLOR_PAIR(CP_TYPE));
            else if (is_directive) attron(COLOR_PAIR(CP_PREPROC));
            else if (is_kw) attron(COLOR_PAIR(CP_KEYWORD) | A_BOLD);
...
void Editor::draw_status(int my, int mx) {
    ...
    if (in_search_mode) {
        ...
        if (std::chrono::duration_cast<std::chrono::seconds>(now - msg_time).count() < STATUS_TIMEOUT) {
            ...
            std::string suf;
            if (!search_results.empty())
                suf = std::format(" | [{}/{}]", search_idx + 1, (int)search_results.size());
...
void Editor::run() {
    ...
    while (running) {
        ...
        if (ch == CTRL('q')) {
            ...
            auto cmd = InsertCommand::make(tab.buffer, std::string(settings.tab_width, ' '), tab.y, tab.x);
            ...
            if (tab.sel_has) {
                ...
                auto cmd = DeleteRangeCommand::make(tab.buffer, a_y, a_x, b_y, b_x);
                tab.history.execute(std::move(cmd));
                tab.y = a_y;
                tab.x = a_x;
                clear_selection(tab);
                ...
                system_copy(tab.clipboard);
                auto cmd = DeleteCommand::make(tab.buffer, tab.clipboard, tab.y, 0);
                tab.history.execute(std::move(cmd));
                tab.x = 0;
            }
            redraw = true;
            ...
            if (!text.empty()) {
                if (tab.sel_has) {
                    ...
                    int a_y, a_x, b_y, b_x;
                    selection_bounds(tab, a_y, a_x, b_y, b_x);
                    auto del = DeleteRangeCommand::make(tab.buffer, a_y, a_x, b_y, b_x);
                    tab.history.execute(std::move(del));
                    tab.y = a_y;
                    tab.x = a_x;
                    clear_selection(tab);
                    auto cmd = PasteCommand::make(tab.buffer, text, tab.y, tab.x);
                    tab.history.execute(std::move(cmd));
                } else if (text.find('\n') != std::string::npos) {
                    auto cmd = PasteCommand::make(tab.buffer, text, tab.y, tab.x);
                    tab.history.execute(std::move(cmd));
                } else {
                    auto cmd = InsertCommand::make(tab.buffer, text, tab.y, tab.x);
                    tab.history.execute(std::move(cmd));
            ...
            if (!input.empty()) {
                ...
                if (ec == std::errc() && ptr == input.data() + input.size()) {
                    if (line < 1 || line > tab.buffer.GetLineCount())
                        status_msg = std::format("Line out of range (1-{})", tab.buffer.GetLineCount());
                    else { tab.y = line - 1; tab.x = 0; status_msg = std::format("Jumped to L:{}", line); }
                } else status_msg = "Invalid line number";
                msg_time = std::chrono::steady_clock::now();
            }
            redraw = true;
        }
            ...
            if (ch == KEY_UP && sidebar_sel > 0) sidebar_sel--;
                ...
                if (!n.empty()) {
                    ...
                    if (f.is_open()) {
                        f.close();
                        set_status(std::format("Created {}", n));
                        update_sidebar();
                    } else {
                        set_status(std::format("Error: cannot create {}", n));
                    }
                }
            } else if (ch == 'd') {
                if (sidebar_sel > 0) {
            ...
            if (ch == KEY_UP && tab.y > 0) tab.y--;
                ...
                if (tab.y < tab.buffer.GetLineCount()) {
                    ...
                    auto cmd = NewLineCommand::make(tab.buffer, tab.y + 1,
                ...
                if (tab.y < tab.buffer.GetLineCount() && tab.x <= (int)tab.buffer[tab.y].length()) {
                    if (ch == '(') {
                        auto cmd = InsertCommand::make(tab.buffer, "()", tab.y, tab.x);
                        tab.history.execute(std::move(cmd));
                        tab.x++;
                    } else if (ch == '{') {
                        auto cmd = InsertCommand::make(tab.buffer, "{}", tab.y, tab.x);
                        ...
                        auto cmd = InsertCommand::make(tab.buffer, std::string(1, (char)ch), tab.y, tab.x);
                        tab.history.execute(std::move(cmd));
                        tab.x++;
                    }
                }
